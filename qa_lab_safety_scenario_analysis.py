...
source=json.loads((PKG/"source.json").read_text(encoding="utf-8"))
...
for name,count in expected.items():
    path=PKG/name; check(path.exists() and path.stat().st_size>1000,f"{name} missing or too small")
    if path.exists():
...
product_text="\n".join((p.extract_text() or "") for p in PdfReader(str(PKG/"product.pdf")).pages)
for x in items:
...
student_text="\n".join((p.extract_text() or "") for p in PdfReader(str(PKG/"student-packet.pdf")).pages)
...
teacher_text="\n".join((p.extract_text() or "") for p in PdfReader(str(PKG/"teacher-guide-and-key.pdf")).pages)
check("ANSWER KEY" in teacher_text.upper() and "CER MODEL" in teacher_text.upper(),"teacher file missing full key or CER model")
preview_text="\n".join((p.extract_text() or "") for p in PdfReader(str(PKG/"preview.pdf")).pages)
...
reader=PdfReader(str(PKG/"product.pdf")); embedded_poppins=0
...
doc=Document(str(PKG/"product-editable.docx"))
...
for name in ("cover.png","listing-01-cover.png","listing-02-inside.png","listing-03-teacher-ready.png"):
    path=PKG/name; check(path.exists(),f"{name} missing")
    if path.exists():
